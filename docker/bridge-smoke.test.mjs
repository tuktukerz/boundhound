...
function smokeContainerRunning() {
  const r = spawnSync("docker", ["inspect", "-f", "{{.State.Running}}", "omop-smoke"], { encoding: "utf8" })
  return r.status === 0 && r.stdout.trim() === "true"
}
const available = smokeContainerRunning()

// Integration test: only runs when the omop-smoke container is up; skipped otherwise
